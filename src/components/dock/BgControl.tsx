import { ColorArea, ColorPicker, ColorSlider, ColorSwatch, Label, Slider, Switch } from '@heroui/react'
import type { Color } from 'react-aria-components'
import { useLogo } from '#/hooks/useLogo'
import type { GradientStop } from '#/store/logoStore'

function ColorSwatchPopover({ value, onChange }: { value: string; onChange: (hex: string) => void }) {
  return (
    <ColorPicker value={value} onChange={(c: Color) => onChange(c.toString('hex'))}>
      <ColorPicker.Trigger>
        <ColorSwatch size="sm" shape="circle" />
      </ColorPicker.Trigger>
      <ColorPicker.Popover placement="top">
        <ColorArea colorSpace="hsb" xChannel="saturation" yChannel="brightness" className="max-w-full">
          <ColorArea.Thumb />
        </ColorArea>
    ...
    <div className="flex w-52 flex-col gap-4">
      ...
      {!isGradient && background.type === 'solid' && (
        <div className="flex items-center justify-between gap-3">
          ...
          <ColorSwatchPopover
            ...
            onChange={(c) => set((d) => { if (d.background.type === 'solid') d.background.color = c })}
          />
        </div>
      )}
      ...
      {isGradient && background.type === 'gradient' && (
        <GradientTab
          ...
          onDirection={(v) => set((d) => { if (d.background.type === 'gradient') d.background.direction = v })}
          onStop={(i, stop) => set((d) => { if (d.background.type === 'gradient') d.background.stops[i] = stop })}
        />
      )}
    </div>
  )
...
function GradientTab({
  ...
  return (
    <div className="flex flex-col gap-4">
      <Slider value={direction} onChange={(v) => onDirection(v as number)} minValue={0} maxValue={360} step={1} aria-label="Gradient angle">
        <div className="flex justify-between">
          <Label className="text-xs text-muted">Angle</Label>
          <Slider.Output className="text-xs text-muted">{() => `${direction}°`}</Slider.Output>
        </div>
