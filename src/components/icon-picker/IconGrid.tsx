...
export function IconGrid({ icons, isLoading, selected, onSelect }: IconGridProps) {
  ...
  return (
    ...
    >
      <div style={{ height: rowVirtualizer.getTotalSize(), width: '100%', position: 'relative', padding: '4px 0' }}>
          ...
          return (
            ...
            >
              {rowIcons.map((iconName) => (
                ...
                >
                  <Icon icon={iconName} width={24} height={24} className="text-(--foreground)" />
                </button>
              ))}
              {/* fill empty slots so justify-between stays even */}
              {Array.from({ length: COLS - rowIcons.length }).map((_, i) => (
                <div key={`empty-${i}`} className="h-12 w-12 shrink-0" />
              ))}
            </div>
          )
        })}
