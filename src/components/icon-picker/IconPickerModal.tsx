...
import { Modal, Label, ListBox, Select, SearchField, Separator } from '@heroui/react'
...
export function IconPickerModal() {
  ...
  const { data: icons = [], isFetching } = useIconSearch(debouncedQuery, prefix)

  const handleSelect = (name: string) => {
    set((d) => { d.iconName = name })
    closeIconPicker()
  }

  return (
    <Modal isOpen={isOpen} onOpenChange={(open) => { if (!open) closeIconPicker() }}>
          ...
          <Modal.Dialog>
            ...
            <Modal.Body className="p-0 overflow-visible">
              <div className="flex gap-2 pt-3">
                <SearchField autoFocus variant='secondary' value={query} onChange={setQuery} className="flex-1" aria-label="Search icons">
                  <SearchField.Group>
