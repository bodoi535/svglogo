import type { LogoState, Background } from '#/store/logoStore'
...
export async function buildCanvasSvg(state: LogoState, size = 512): Promise<string> {
  const {
    iconName,
    iconColor,
    iconSize,
  ...
  let iconSvgContent = ''
  try {
    const res = await fetch(`https://api.iconify.design/${prefix}/${name}.svg?color=${encodeURIComponent(iconColor)}&width=${iconPx}&height=${iconPx}`)
